...
class NvidiaService:
    def __init__(self):
        self.api_key=os.getenv("NVIDIA_API_KEY","");self.base_url=os.getenv("NVIDIA_BASE_URL","https://integrate.api.nvidia.com/v1");self.default_model=os.getenv("NVIDIA_DEFAULT_MODEL","nvidia/nemotron-3-nano-30b-a3b");self.default_code_model=os.getenv("NVIDIA_CODE_MODEL","mistralai/codestral-22b-instruct-v0.1");self.client=AsyncOpenAI(api_key=self.api_key,base_url=self.base_url) if self.api_key else None
